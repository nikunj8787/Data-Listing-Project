...
def insert_sample_data(cursor):
    ...
    try:
        ...
        for user in users:
            cursor.execute('''
            INSERT OR IGNORE INTO users (email, role, operator_id, plan_start_date, plan_end_date, plan_type, is_verified)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', user)
        ...
        for prop in properties:
            cursor.execute('''
            INSERT OR IGNORE INTO properties (property_type, location, address, price, bhk_type, area, furnished_status, 
...
def log_contact_view(customer_email, property_id):
    ...
    try:
        ...
        print(f"Error logging contact view: {e}")
